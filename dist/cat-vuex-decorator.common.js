...
var fnKeysType = function (fnKeys) {
    if (!fnKeys) {
...
function createBindingFnVuex(bindType, vMapFn) {
    ...
    return function makeDecorator(fnKeys, n) {
        var tempFnKeys = fnKeysType(fnKeys);
        if (!tempFnKeys) {
            return createDecorator(function (_) { });
        }
        var paramData = tempFnKeys.data;
        var paramType = tempFnKeys.type;
        return createDecorator(function (options, key) {
            if (!options[bindType]) {
                options[bindType] = {};
            }
            var bindMapFn = n ? vMapFn(n, paramData) : vMapFn(paramData);
            var mapObject = new Map();
            var singleBind = false;
            // Handle the array data type.
            if (paramType === typeEnum.array) {
                var tempArr = paramData;
            ...
            if (!singleBind) {
                options[bindType][key] = function () { };
                // Injection function callback processing.
                options[bindType][key] = function wrapperMethod() {
                    var this$1 = this;

                    var bindThis = {};
                    mapObject.forEach(function (vkey, label) {
                        var tempKey = paramType === typeEnum.array ? vkey : label;
                        bindThis[tempKey] = this$1[label];
                    });
                    return bindThis;
                };
            }
